...
(*module Make (C : Communication.S) = struct *)
  module MyMap = Map.Make(struct
    type t = Nfmpi_communication.gid Nfmpi_communication.tag
    let compare = compare
  end)

  type tag = Nfmpi_communication.gid Nfmpi_communication.tag
  type msg = Nfmpi_communication.msg
  type msg_queue = {
  ...
  type callback = Nfmpi_communication.msg -> unit
  type callback_kind = Once | Forever
  type dispatcher = {
  ...
  let call_callback d tag s =
    try
      let f, kind = MyMap.find tag d.d_handlers in
        if kind = Once then
          d.d_handlers <- MyMap.remove tag d.d_handlers;
        print_debug "Calling callback for tag: %a@." Nfmpi_communication.print_tag tag;
        f s
    with
      | Not_found -> print_debug "%a: Received unexpected tag: %a@." Nfmpi_communication.print_here () Nfmpi_communication.print_tag tag
  ...
  let print_queue ff q =
    List.iter (fun (tag, _) -> Format.fprintf ff "   %a" Nfmpi_communication.print_tag tag) q.q_queue;
    Format.fprintf ff "@."
  ...
  let recv_given_msg q tag =
    let rec aux () =
      if q.q_queue = [] then
        Condition.wait q.q_queue_filled q.q_mutex;
      print_debug "Looking for requested tag '%a' @." Nfmpi_communication.print_tag tag;
    ...
    let msg = aux () in
    Mutex.unlock q.q_mutex;
    print_debug "Received the awaited message with tag '%a'@." Nfmpi_communication.print_tag tag;
    msg
  ...
  let recv_n_given_msg q tag n =
    ...
    aux ();
    Mutex.unlock q.q_mutex;
    print_debug "Received the n awaited messages with tag '%a'@." Nfmpi_communication.print_tag tag
  ...
  let dispatch_given_msg q d tag =
    ...
    print_debug "Received %d matching messages wit tag '%a'@." (List.length found)  Nfmpi_communication.print_tag tag
  ...
  let receive q =
    while q.q_alive do
      let tag, msg = Nfmpi_communication.receive () in
      Mutex.lock q.q_mutex;
