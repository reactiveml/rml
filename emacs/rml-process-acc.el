...
;  (define-key rml-mode-map "\C-x`" 'rml-next-error)
  ...
  (if running-xemacs nil ; if not running xemacs
    (let ((map (make-sparse-keymap "Rml"))
      ...
      (define-key rml-mode-map [menu-bar rml] (cons "Rml" map))
      ;; rml-help
    ...
    (if (string-match "^\\(.*\\)\\.\\(ml\\|mli\\)$" name)
        (find-file
         (concat
          ...
          (if (string= "ml" (rml-match-string 2 name)) ".mli" ".ml"))))))
...
(defun rml-find-in-match ()
  (let ((unbalanced 1) (kwop t))
	...
	(cond
	 ...
	 ((string= kwop "in")
	  (if (rml-find-kwop "process")
	      (progn
		(setq beg (point))
		(goto-char pos)
		(re-search-backward "{" beg) ;peut-etre le mettre dans un if?
	    ...
	    (goto-char pos)
	    (setq unbalanced (1+ unbalanced))))
	 ((= fin ?\;)) ;par ex dans le cas await s;
	 (t (setq unbalanced (1- unbalanced))))))
    kwop))
