...
val make : 
  ?seed:(int) ->      (* to fix a seed for the pseudo-random generator *)
  ?fair:(bool) ->     (* favour fairness over efficiency *)
...
val step : ?mode:(step_mode) -> state -> inputs -> state * solution

(** side-effect version of step *)
val step_se : ?mode:(step_mode) -> state ref -> inputs -> solution
