...
(* ------------------------------------------------------------------------ *)
    ...
    let rml_default evt = R.Event._default evt
...
(**************************************)
    let rml_get' evt p =
      fun f_k ctrl jp cd _ ->
        let f_get _ =
          let x = if R.Event.status false evt then R.Event.value evt else R.Event._default evt in
...
(**************************************)
    ...
    let rml_signal_combine ce _default comb p =
      fun f_k ctrl jp cd _ ->
        let evt = R.Event.new_evt_combine (eval_clock_expr cd ce) (_default()) (comb()) in
...
(**************************************)
    ...
    let rml_until expr_evt p =
      fun f_k ctrl jp cd ->
        let new_ctrl = new_ctrl (fun () -> false) (Kill f_k) in
        let f = p (R.end_ctrl new_ctrl f_k) new_ctrl None cd in
        fun _ ->
          let evt = expr_evt () in
          R.set_condition new_ctrl (fun () -> R.Event.status true evt);
    ...
    let rml_until_conf expr_cfg p =
      fun f_k ctrl jp cd ->
        ...
        fun _ ->
          ...
          R.set_condition new_ctrl (fun () -> R.Event.cfg_status true evt_cfg);
          start_ctrl ctrl new_ctrl;
          f unit_value
...
(**************************************)
    ...
    let rml_until_handler_local expr_evt matching_opt p p_handler =
      fun f_k ctrl jp cd ->
        ...
        in
        let new_ctrl = new_ctrl (fun () -> false) (Kill_handler handler) in
        let f = p (R.end_ctrl new_ctrl f_k) new_ctrl None cd in
        fun _ ->
          evt := expr_evt ();
          (match matching_opt with
            | None -> R.set_condition new_ctrl (fun () -> R.Event.status true !evt)
            | Some matching -> R.set_condition new_ctrl (fun () -> R.Event.status true !evt && matching (R.Event.value !evt))
          );
          start_ctrl ctrl new_ctrl;
          f unit_value
      ...
      fun f_k ctrl jp cd ->
        ...
        | Some matching ->
            R.set_condition new_ctrl
              (fun () -> R.Event.status true evt && matching (R.Event.value evt))
        );
        fun () ->
          start_ctrl ctrl new_ctrl;
          f ()
...
(**************************************)
    let step_control_static cond p =
      fun f_k ctrl jp cd ->
        let new_ctrl = new_ctrl (fun () -> false) Susp in
        let f = p (R.end_ctrl new_ctrl f_k) new_ctrl None cd in
    ...
    let rml_control' evt p =
      step_control_static (fun () -> R.Event.status true evt) p

    let rml_control expr_evt p =
      let cond () =
        let evt = expr_evt () in
        fun () -> R.Event.status true evt
      in
      step_control cond p

    let rml_control_match' evt matching p =
      step_control_static
        (fun () -> R.Event.status true evt && matching (R.Event.value evt)) p
    ...
    let rml_control_conf expr_cfg p =
      let cond () =
        let evt_cfg = expr_cfg () in
        fun () -> R.Event.cfg_status true evt_cfg
      in
      step_control cond p
...
(**************************************)
    ...
    let step_when f_k ctrl jp cd p evt =
      ...
      let new_ctrl = R.new_ctrl (fun () -> false) When in
      let rec when_act _ =
      ...
      fun () ->
        ...
        R.set_condition new_ctrl (fun () -> R.Event.status false evt);
