...
type join_point = R.Join.join_point option
...
let rml_global_signal_combine k _default combine =
  R.Event.new_evt_global k _default combine
...
let rml_weak_pause' pause_ce =
  fun f_k ctrl jp cd _ ->
    let pause_ck = eval_clock_expr cd pause_ce in
    R.on_eoi pause_ck (fun () -> R.on_next_instant_kind Weak ctrl f_k)
...
let rml_get' evt p =
  fun f_k ctrl jp cd _ ->
    let f_get _ =
      let x = if R.Event.status evt then R.Event.value evt else R.Event._default evt in
...
let _rml_signal f_k ctrl jp cd  kind ce re _default combine reset p =
  ...
  R.Event.new_evt cd ck r kind _default combine reset k ()

let rml_signal_combine kind ce re _default combine reset p =
  fun f_k ctrl jp cd _ ->
    _rml_signal f_k ctrl jp cd kind ce re (_default ()) (combine ()) reset p
