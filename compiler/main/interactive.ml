...
let compile_decl_list module_name itf info_chan out_chan decl_list =
  ...
  in
  (* the implementation *)
  Reac2code.compile_implementation_back_end info_chan out_chan module_name
    intermediate_code
...
let init () =
  Compiler_options.set_init_stdlib ();
...
let translate_phrase phrase =
  ...
  try
    ...
    (* the implementation *)
    let ocaml_code =
      Reac2code.compile_implementation_back_end_buf !Misc.err_fmt module_name
        intermediate_code
    in
    None, ocaml_code
  with x ->
